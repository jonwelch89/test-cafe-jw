...
'@test'['act.click, array parameter, ok() assertion'] = {
    'Click two checkbox labels': function () {
        var labels = [$(':containsExcludeChildren(Support for testing on remote devices)'), $(':containsExcludeChildren(Reusing existing JavaScript code for testing)')];
        act.click(labels);
    },
...
'@test'['act.select example'] = {
    ...
    '3.Confirm input selection state': function () {
        eq(getInput()[0].selectionStart, 1);
        eq(getInput()[0].selectionEnd, 7);
        if (getInput()[0].selectionDirection)
            eq(getInput()[0].selectionDirection, 'backward');
    }
};
